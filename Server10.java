...
public class Server10{    
   public static void main(String args[]) {     	
    ...
    try{	    	
        aSocket = new DatagramSocket(20010);
	    byte[] buffer = new byte[1000]; 			
	   	System.out.println("Server10 is ready and accepting clients' requests ... ");
		while(true){ 				
...
public static String calculateBlockHash(int blockNum,String data ,int nonce) {
   ...
   public static int findNonce(int blockNumber, String data){
    int nonce = 450001;
            ...
            while(!output.substring(0, 4).equals("0000")&& nonce<500001) {
                nonce++;
                output = calculateBlockHash(blockNumber, data, nonce); 
                }
                if (nonce == 500001 ) nonce = 0;
