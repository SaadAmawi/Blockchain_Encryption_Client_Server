...
public class Server9{    
   public static void main(String args[]) {     	
    ...
    try{	    	
        aSocket = new DatagramSocket(20009);
	    byte[] buffer = new byte[1000]; 			
	   	System.out.println("Server9 is ready and accepting clients' requests ... ");
		while(true){ 				
...
public static String calculateBlockHash(int blockNum,String data ,int nonce) {
   ...
   public static int findNonce(int blockNumber, String data){
    int nonce = 400001;
            ...
            while(!output.substring(0, 4).equals("0000")&& nonce<450001) {
                nonce++;
                output = calculateBlockHash(blockNumber, data, nonce); 
                }
                if (nonce == 450001 ) nonce = 0;
